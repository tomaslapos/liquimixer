...
### 6. ✅ FUNKCE KTERÉ FUNGUJÍ - NETESTOVAT ZNOVU
...
### 10.02.2026 - Opravy bugů z předchozího dne

**Opraveno:**
- ✅ `saveFlavorToFavorites()` - přidáno chybějící pole `share_url` (způsobovalo selhání INSERT do `favorite_products`)
- ✅ Počáteční barva flavor slideru - přidáno přímé nastavení barvy tracku v `showFlavorSliderWithRange()` při výběru konkrétní příchutě
- ✅ Překlady ingredientů - přidány case `sweetener` a `shortfill_base` do `getIngredientName()` pro starší recepty
- ✅ Přidán klíč `ingredients.shortfill_base` do `cs.json` a `en.json`

**Poznámky k opravám:**
- Problém s ukládáním příchutí do oblíbených NEBYL v RLS policy, ale v chybějícím poli `share_url` v INSERT
- Barva slideru se nyní nastavuje přímo v `showFlavorSliderWithRange()` bez čekání na event handler

**K testování:**
- Uložení receptu s příchutí z databáze příchutí (ověřit že se uloží do oblíbených)
- Zobrazení starších receptů se `sweetener` a `shortfill_base` ingrediencemi
...
**Service Worker:** `liquimixer-v183`
...
- **Aktuální verze:** `liquimixer-v184`
