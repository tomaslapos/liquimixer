...
1. **Opravit překlady ingredientů v detailu receptu**
   - `sweetener` se nepřekládá (Med, Melasa, Cukr)
   - `shortfill_base` se nepřekládá
   - Příchutě se zobrazují s mixem jazyků
   - **Řešení:** Upravit `getIngredientName()` v `app.js`
...
4. **Databáze příchutí od výrobců** (viz plánované funkce)
...
- **Aktuální verze:** `liquimixer-v163`
