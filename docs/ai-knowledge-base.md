...
# Jazyk: čeština. AI překládá do jazyka uživatele. Aktualizováno: 10.03.2026
...
Úprava hotového komerčního tabáku — přidání glycerinu, příchutě, sladidla nebo melasy pro zlepšení.

**Postup:** Vybrat tabák (z oblíbených nebo ručně) → Množství tabáku (g) → Co chci řešit? (checkboxy):
- **Tabák je suchý, málo dýmí** → přidat glycerin (VG), slider 0-30%
- **Tabák málo chutná** → přidat flavor drops/booster (2-5 kapek na 20g)
- **Chci změnit/přidat příchuť** → přidat vape koncentrát příchutě (1-4 příchutě, každá s %, kategorií, autocomplete z DB)
- **Chci přidat nikotin** → typ nikotinu (freebase/sůl), síla báze, cílový nikotin (slider)
- **Mixology — pokročilé úpravy** → pokročilé přísady (viz níže)

**Mixology — pokročilé přísady (Tobacco Tweak):**
Uživatel může vybrat libovolnou kombinaci těchto přísad:
| Přísada | Slider | Výchozí | Rozsah | Účel |
|---|---|---|---|---|
| **Med** | % z tabáku | 5% | 0-20% | Zvýší sladkost a hustotu |
| **Melasa** | % z tabáku | 10% | 0-30% | Tradiční sladidlo, lepivost |
| **Mentol / Cooling** | kapky na 20g | 3 | 0-10 | Chladivý efekt |
| **Kyselina citronová** | g na 20g | 0.5g | 0-5g | Zvýrazní ovocné tóny |
| **Voda / ovocná šťáva** | % z tabáku | 5% | 0-20% | Rozředění, vlhčení |

Výsledek: tabulka složek s přesnými dávkami (ml, g, kapky). Poznámky k přípravě a době zrání.
...
## VG/PG poměr příchutě — Slider vs Složení příchutě (Liquid PRO)
V Liquid PRO má každá příchuť DVĚ možnosti nastavení VG/PG:

1. **VG/PG slider** (výchozí, jednoduchý) — posuvník 0-100% VG. Kalkulátor z něj odvodí složení: PG = 100% - VG%, alkohol/voda/ostatní = 0%. Toto je **zjednodušený** režim.
2. **Složení příchutě** (pokročilý panel) — uživatel klikne na "Složení příchutě" a otevře panel s detailními hodnotami PG/VG/alkohol/voda/ostatní (součet = 100%).

**PRAVIDLA PRIORITY:**
- Pokud je panel "Složení příchutě" **otevřený** (rozbalený) → použije se **explicitní složení z panelu** a slider se ignoruje
- Pokud je panel **zavřený** (sbalený) → použije se **hodnota ze slideru** (zjednodušený VG/PG poměr)
- Tato logika funguje **stejně pro dynamicky přidávané příchutě 2, 3, 4**
- Tato logika funguje **stejně pro aditiva** (chladivo, sladidlo, zesilovač, terpeny)
- Při **uložení receptu** se do databáze ukládá složení POUZE pokud uživatel explicitně otevřel panel složení. Pokud použil jen slider, složení se neukládá (použije se výchozí odvozené ze slideru).
- Při **editaci receptu** — pokud recept obsahuje uložené složení, panel se automaticky otevře a předvyplní. Pokud ne, slider se předvyplní uloženou hodnotou.
...
**Příchutě v oblíbených:** Uživatel může uložit vape příchutě i shisha tabáky. Příchutě mohou být:
- **S vazbou na veřejnou databázi** — propojení na konkrétní příchuť z DB (automatické předvyplnění %, doby zrání, výrobce)
- **Bez vazby na databázi** — uživatel ručně zadá název, výrobce a parametry (typicky méně známé/lokální značky)
- Obě varianty se zobrazují v autocomplete dropdown při vyhledávání v kalkulátoru
